...
import test2_sensor_controler                     # plek waar alle oude code stond
import warnings
import threading
...
def main():
    ...
    while iteration < iterations:
        try:
            threadUpdate = threading.Thread(target=test2_sensor_controler.updateDataframe, args=(iteration, animationQueue,))
            threadUpdate.start()
            
            time.sleep(1 / dataFrequency)       # Runs every 1/f period        
        except KeyboardInterrupt:
            test2_sensor_controler.writeData(path=path, use_syringe=use_syringe)
            break
